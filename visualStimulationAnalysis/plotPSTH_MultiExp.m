            ...
            for ni = 1:numel(eNeurons)
                ...
                for ci = 1:nCats
                    ...
                    if ~isSplitActive
                        ...
                        targetVal = str2double(catLabelsAll{s}(ci));
                        trialMask = ismembertol(catValues, targetVal, 1e-3);
                    end
...
for s = 1:nStim
    ...
    for ci = 1:nc
        ...
        % Which base color
        baseIdx = mod(ci-1, size(basePalette,1)) + 1;

        % Alternate between dark/light shade
        shadeIdx = floor((ci-1) / size(basePalette,1));

        baseC = basePalette(baseIdx,:);
        ...
        if shadeIdx == 0
            ...
            % lighter version
            newC = baseC + (1-baseC)*0.45;
        end

        cmap(ci,:) = min(max(newC,0),1);
    ...
    for b = 1:nDepthBins
        for ci = 1:numel(catLabelsAll{s})
            ...
            elseif isSplitHere
                ...
                legendLabel = sprintf('%s (n=%d)', ...
                     catLabelsAll{s}(ci), nValid);
            else
