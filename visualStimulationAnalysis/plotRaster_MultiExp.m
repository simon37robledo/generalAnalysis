...
arguments
    ...
    params.speed           string        = "best"                         % ball-speed selector: "max" or other
   ...
   % --- Preferred-category PSTH content (independent of sorting) ---
    params.prefCatPSTH     logical       = false                         % if true: each panel shows each neuron's preferred-category PSTH 
                                                                         % (all trials of preferred level), via splitCategory/splitLevels; 
                                                                         % compatible with unionUnits (content only, does not reorder rows)
    % --- Stats-driven preferred-category PSTH (matches the AllExpAnalysis swarm) ---
    params.statPrefCatPSTH  logical      = false                         % if true: build each neuron's PSTH from its STORED prefCat trials (StatisticsPerNeuron), so the PSTH shows exactly the category the swarm's ZScoreU came from
...
% statPrefCatPSTH (stats prefCat) and prefCatPSTH (empirical argmax over splitCategory)
% are two different ways to pick the displayed category — forbid using both at once.
assert(~(params.statPrefCatPSTH && params.prefCatPSTH), ...              % they would fight over PSTH content
    'statPrefCatPSTH and prefCatPSTH are mutually exclusive — pick one preferred-category source.');
...
if params.unionUnits
    ...
    assert(params.sortBy ~= "preferredCategory", ...
        ['unionUnits = true is incompatible with sortBy = "preferredCategory" ' ...
         '(per-stim sort breaks row alignment). For preferred-category content ' ...
         'with union sorting, use prefCatPSTH = true instead.']);

    % splitCategory under union is allowed ONLY for preferred-category PSTH
    % content (prefCatPSTH=true), which changes what each panel displays but
    % not the row order. It remains incompatible with preferredCategory
    % *sorting* (blocked above), which would reorder rows per stim.
    if ~params.prefCatPSTH
        assert(all(strlength(params.splitCategory) == 0) || ...
               (isscalar(params.splitCategory) && params.splitCategory == ""), ...
            'unionUnits = true requires empty splitCategory unless prefCatPSTH = true.');
    end
...
% -------------------------------------------------------------------------
if params.sortBy == "preferredCategory" || params.prefCatPSTH
...
% -------------------------------------------------------------------------
NP_first = loadNPclassFromTable(exList(end));                              % load NP object for path info
...
if params.MarkFPnFN
    csvFile = strrep([saveDir nameOfFile], '.mat', '_falseNegPos.csv');
    if exist(csvFile, 'file')
        opts = detectImportOptions(csvFile);
        
        % FIX: Use setvartype to safely force the Verdict column to a string
        opts = setvartype(opts, 'Verdict', 'string'); 
        
        D_fpfn = readtable(csvFile, opts);
        fprintf('Loaded FP/FN labels from: %s\n', csvFile);
    else
        warning('MarkFPnFN is true, but map file was not found:\n%s\nRun findFalseNegAndPos first.', csvFile);
        params.MarkFPnFN = false; % Turn off if file missing
    end
end
...
if forloop
    ...
    for ei = 1:nExp
        ...
        expBestSpeed   = cell(1, nStim);                                 % per-neuron winning-speed index (MB/MBR)
        expSpeedC      = cell(1, nStim);                                 % per-speed condition matrices    (MB/MBR)
        ...
        for s = 1:nStim
            ...
            if params.statType == "BootstrapPerNeuron"
                ...
                % Bare call: loads the last saved StatisticsPerNeuron result
                % as-is. This deliberately does NOT pass computation params.
                % StatisticsPerNeuron's nargin==1 guard treats a bare call as
                % "return whatever was last computed", so it will NOT recompute
                % (or overwrite) even when the cache was built with non-default
                % params (e.g. AllExpAnalysis run with a focused
                % durationWindow/CategoryLevels/Lock). This keeps
                % plotRaster_MultiExp a read-only consumer.
                %
                % NOTE: must be a bare method call, not obj.getAnalysisFileName
                % from here — getAnalysisFileName derives its filename from the
                % *calling* method's name via dbstack, so calling it directly
                % from plotRaster_MultiExp resolves to the wrong file
                % (plotRaster_MultiExp.mat instead of StatisticsPerNeuron.mat).
                Stats = obj.StatisticsPerNeuron;                        % load cached permutation test (flags come from the cache's own S.params)
            end
            ...
            if (stimType == "MB" || stimType == "MBR") && params.speed == "best"
                % BEST-SPEED: mirror AllExpAnalysis — take the minimum p across
                % all Speed* fields, and record each neuron's winning speed so
                % its PSTH is built from that speed's trials.
                ...
                pvals           = pvals(:)';                             % row vector (matches downstream usage)
                expBestSpeed{s} = bestSp(:)';                            % store winning speed index per neuron
                expSpeedC{s}    = Cspeed;                                % store all speeds' condition matrices
                ...
                expBestSpeed{s} = ones(1, numel(pvals));                % trivial single-speed index
                expSpeedC{s}    = {C};                                   % single condition matrix
            end
            expPvals{s} = pvals;                                         % store p-value vector
            expC{s} = C;                                                 % store representative condition matrix
            expDirectimes{s} = C(:, 1)' + startStim;                    % representative onsets (best speed applied per-neuron in PSTH loop)
        ...
        for s = 1:nStim
            ...
            directimesSorted = expDirectimes{s};                         % representative onset times (ms)
            C              = expC{s};                                    % condition matrix
            bestSpeed      = expBestSpeed{s};                            % per-neuron winning-speed index
            speedC         = expSpeedC{s};                               % per-speed condition matrices
            startStimMs    = expStartStim(s);                           % onset offset (ms; 0 except SDGm)

            % Stimulus-parameter names for prefCat decoding (statPrefCatPSTH only).
            colNamesStim = strings(1,0);                                 % default empty (unused unless decoding)
            if params.statPrefCatPSTH                                    % only pay this cost when needed
                NRcn = obj.ResponseWindow;                              % response-window struct (for colNames)
                try,   cnAll = NRcn.(fieldName).colNames{1};           % sub-field column names (Speed*/Moving/Static)
                catch, cnAll = NRcn.colNames{1}; end                   % flat struct fallback (RG, NI, ...)
                colNamesStim = string(cnAll(5:end));                   % strip 4 bookkeeping cols -> parameter names
            end
            ...
            isCatMode = (params.sortBy == "preferredCategory" || params.prefCatPSTH) && ...
            ...
            if isCatMode
                ...
                % ==========================================================
               if ~params.useGeneralFilter && ~params.unionUnits
                    gt = detectGratingType(stimType);                    % '' for non-grating, 'moving'/'static' for SDG

                    % --- Auto-detect Lock from the existing per-category cache ---
                    % StatisticsPerNeuronPerCategory supports a Lock pre-filter
                    % (mirrors CategoryLevels/Lock in StatisticsPerNeuron). Rather
                    % than adding a redundant plotRaster_MultiExp parameter, peek
                    % at this stimulus/category's cached results file and reuse
                    % whatever Lock it was built with — the same auto-detection
                    % pattern statPrefCatPSTH already uses for focusSpec. If no
                    % cache exists yet, default to no lock (unchanged behaviour).
                    %
                    % Build the path directly from the analysis folder rather
                    % than obj.getAnalysisFileName: getAnalysisFileName derives
                    % its name from the *calling* method via dbstack, so calling
                    % it from here would resolve to plotRaster_MultiExp.mat, not
                    % StatisticsPerNeuronPerCategory's file. This reconstructs
                    % StatisticsPerNeuronPerCategory's outputFile naming exactly:
                    % <folder>\StatisticsPerNeuronPerCategory_<category>.mat.
                    detectedLock = {};                                   % default: no lock
                    catCacheFile = fullfile(obj.visualStimAnalysisFolder, ...
                        ['StatisticsPerNeuronPerCategory_' lower(strtrim(char(thisCatName))) '.mat']);
                    if isfile(catCacheFile)
                        cachedCat = load(catCacheFile, 'params');        % peek at saved params only
                        if isfield(cachedCat, 'params') && isfield(cachedCat.params, 'Lock')
                            detectedLock = cachedCat.params.Lock;        % reuse the Lock the cache was built with
                            if ~isempty(detectedLock)
                                fprintf('  [%s] exp %d: auto-detected Lock from cache for "%s".\n', ...
                                    stimType, ex, thisCatName);
                            end
                        end
                    end
                    ...
                    catStatsArgs = { ...
                        ...
                        'Lock',            detectedLock};                % auto-detected focus lock (see above)
                    if ~isempty(gt)
            ...
            for ni = 1:numel(eNeurons)
                ...
                bMean = NaN;                                             % sentinel: set in catMode block or lazily in standard mode
                bStd  = NaN;

                if isCatMode && params.zScore

                    % All-trials matrix (baseline is stimulus-independent, so
                    % pooling all trials gives the most stable estimate).
                    nTrialsAll = numel(directimesSorted);                % total number of trials
                    ...
                    MRall = reshape(MRall, nTrialsAll, []);              % [nTrialsAll × time]; reshape avoids singleton collapse

                    % Across-trial baseline statistics from per-trial baseline
                    % means. Matches sdBase in StatisticsPerNeuron and, unlike
                    % the SD across PSTH bins, does not shrink with trial count,
                    % so panels with different trial counts stay on a comparable
                    % z-scale (critical for the shared colorbar).
                    perTrialBaseAll = mean(MRall(:, 1:lockedPreBase), 2) * 1000; % [nTrialsAll × 1] spk/s
                    bMean = mean(perTrialBaseAll);                       % across-trial baseline mean
                    bStd  = std(perTrialBaseAll);                        % across-trial baseline SD

                    % Silent baseline (bStd==0): keep the neuron. The data-driven
                    % SD floor applied after the experiment loop prevents an
                    % infinite z-score, so no drop / NaN placeholder is needed.
                    if bStd == 0
                        nDroppedZeroSD(s) = nDroppedZeroSD(s) + 1;      % tally only (neuron retained and floored later)
                    end
                end
                ...
                if isCatMode
                    ...
                    % ==================================================
                    % STANDARD MODE: PSTH from best-speed trials, optionally
                    % restricted to the neuron's stored preferred category.
                    % ==================================================
                    prefLevel = NaN;                                     % not applicable in standard mode

                    Cu = speedC{ bestSpeed(u) };                         % winning speed's condition matrix
                    % Grid-routed MB has no trial-level category to decode (its
                    % category is a grid cell). The grid path stores no prefCat, so
                    % its ABSENCE in the sub-struct is the grid test — no flag needed.
                    sub      = statSubStruct(expStats{s}, stimType, bestSpeed(u));  % relevant stats sub-struct
                    canDecode = isfield(sub, 'prefCat') && ~isempty(sub.prefCat);   % standard path -> decodable

                    if params.statPrefCatPSTH && canDecode             % show the swarm's preferred category
                        sub = statSubStruct(expStats{s}, stimType, bestSpeed(u));   % relevant stats sub-struct
                        selRows = preferredCatRows(sub, Cu, colNamesStim, u);       % trial rows of prefCat(u)
                        onsetsU = Cu(selRows, 1)' + startStimMs;         % preferred-category onsets (ms)

                        % Baseline from ALL trials (stimulus-independent -> stable
                        % z-scale, matching the swarm's pooled sdBase). Set bMean/bStd
                        % here so the deferred z-score block below does not recompute
                        % them from the preferred-category subset.
                        if params.zScore
                            allOns = Cu(:, 1)' + startStimMs;           % every trial's onset
                            MRbase = BuildBurstMatrix(goodU(:, u), round(p_sort.t), ...
                                round(allOns - lockedPreBase), round(windowTotal));  % all-trial matrix
                            MRbase = reshape(MRbase, numel(allOns), []); % [nTrialsAll x time]
                            ptb    = mean(MRbase(:, 1:lockedPreBase), 2) * 1000;     % per-trial baseline (spk/s)
                            bMean  = mean(ptb);                          % across-trial baseline mean
                            bStd   = std(ptb);                           % across-trial baseline SD
                            if bStd == 0, nDroppedZeroSD(s) = nDroppedZeroSD(s) + 1; end  % silent baseline: floored later
                        end
                    else
                        onsetsU = Cu(:, 1)' + startStimMs;               % all best-speed trials (grid MB or no decode)
                    end
                    ...
                    MRhist = BuildBurstMatrix( ...
                        goodU(:, u), ...                                 % spike identity for this unit
                        round(p_sort.t), ...                             % rounded sample timestamps
                        round(onsetsU - lockedPreBase), ...              % trial start = onset - baseline
                ...
                % --- Baseline stats for the DEFERRED z-score ---
                % Division is postponed to a post-loop block so the SD floor can
                % be the 15th pct of all non-zero baseline SDs (data-driven).
                if params.zScore && isnan(bMean)                         % standard mode: baseline not set in catMode
                    perTrialBase = mean(MRhist(:, 1:lockedPreBase), 2) * 1000; % per-trial baseline rate (spk/s)
                    bMean = mean(perTrialBase);                          % across-trial baseline mean
                    bStd  = std(perTrialBase);                          % across-trial baseline SD
                end
                if params.zScore && bStd == 0                           % silent baseline: keep, tally for floor report
                    nDroppedZeroSD(s) = nDroppedZeroSD(s) + 1;          % counts neurons that will hit the SD floor
                end
                % neuronPSTH stays in spk/s here; z-scoring happens after the loop.
                ...
                rasterAll{s}    = [rasterAll{s}; neuronPSTH];           % raw PSTH row (z-scored after the loop)
    ...
    % Report zero-SD neurons (kept, not dropped — normalised against the SD floor)
    for s = 1:nStim
        if nDroppedZeroSD(s) > 0
            fprintf('  [%s] %d neuron(s) with zero baseline SD — retained and floored.\n', ...
    ...
    % Guard: if zero neurons were collected across every experiment and
    % stimulus (e.g. every experiment skipped for a missing stim type / a
    % union-mode Phy-ID mismatch, or no neuron passing the alpha threshold),
    % the sort-by-group logic below would otherwise crash with a cryptic
    % "0 elements on the right-hand side" error (respGroupAll{refStim}
    % evaluating to an empty comma-separated list). Fail clearly here instead.
    totalNeurons = sum(cellfun(@(r) size(r,1), rasterAll));
    assert(totalNeurons > 0, 'plotRaster_MultiExp:noNeurons', ...
        ['No responsive neurons were collected across any experiment/stimulus. ' ...
         'Check the console output above for per-experiment skip warnings — ' ...
         'this usually means the requested stimulus types were not present ' ...
         '(or no neurons passed alpha) for the experiments in exList.']);
    ...
    % ------------------------------------------------------------------
    % DEFERRED Z-SCORE WITH DATA-DRIVEN SD FLOOR
    % Subtract each neuron's baseline mean and divide by max(its baseline SD,
    % sdFloor). sdFloor is the params.sdFloorPrctile-th percentile of all
    % strictly-positive baseline SDs pooled across panels, so silent-baseline
    % neurons are kept and normalised against the quietest real cells rather
    % than producing an infinite z. clim clipping caps the display.
    % ------------------------------------------------------------------
    sdFloor = NaN;                                                       % default when z-scoring is off
    if params.zScore
        allBStd = [];                                                   % pool non-zero baseline SDs across panels
        for s = 1:nStim
            bs = bStdAll{s};                                            % baseline SDs for this panel
            allBStd = [allBStd, bs(bs > 0 & isfinite(bs))]; %#ok<AGROW> % keep strictly-positive, finite values
        end
        if isempty(allBStd)
            sdFloor = 1;                                                % degenerate fallback (every neuron silent)
            warning('plotRaster:noNonzeroSD', 'No non-zero baseline SDs; using sdFloor = 1 spk/s.');
        else
            sdFloor = prctile(allBStd, params.sdFloorPrctile);          % data-driven floor
        end
        fprintf('Z-score SD floor (%gth pctl of non-zero baseline SDs) = %.3f spk/s.\n', ...
            params.sdFloorPrctile, sdFloor);

        for s = 1:nStim
            if isempty(rasterAll{s}); continue; end                    % nothing to normalise
            denom = max(bStdAll{s}(:), sdFloor);                       % floored denominator per neuron [nNeu x 1]
            rasterAll{s} = (rasterAll{s} - bMeanAll{s}(:)) ./ denom;   % deferred z-score (broadcast over bins; NaN rows stay NaN)
        end
    end
    ...
    S.sdFloor         = sdFloor;                                         % data-driven z-score SD floor (spk/s) for reproducibility
...
% --- FP/FN marker styling and running totals (used inside the tile loop, and for the legend built after it) ---
if params.MarkFPnFN
    colorFN       = [0      0.4470 0.7410];                              % blue  -- unambiguous vs. orange FP
    colorFP       = [0.8500 0.3250 0.0980];                              % orange -- unambiguous vs. blue FN
    colorMismatch = [0      0      0];                                   % black 'x' -- color AND shape both differ from FN/FP asterisks
    nFNtotal       = 0;                                                  % running count of FN markers drawn (all panels)
    nFPtotal       = 0;                                                  % running count of FP markers drawn (all panels)
    nMismatchTotal = 0;                                                  % running count of cache-mismatch markers drawn
end
...
for s = 1:nStim
    ...
    if params.MarkFPnFN
        % Extract the numeric vector for the current stimulus panel
        currentPhy = phyAll{s};
        currentExp = expAll{s};

        % Get the right edge of the current plot axis so markers sit at the end
        xLimits = xlim;
        xEnd = xLimits(2) + (0.01 * diff(xLimits)); % Position 1% past the right edge

        % Loop through every neuron row displayed in the current raster
        for i = 1:numel(currentPhy)
            % Match this row to its entry in the FP/FN table
            idx = find(D_fpfn.Exp == currentExp(i) & D_fpfn.PhyID == currentPhy(i), 1);

            if ~isempty(idx)
                % Check if this specific stimulus panel is flagged as an FN or FP
                isFN = (D_fpfn.FN_for(idx) == params.stimTypes(s));
                isFP = (D_fpfn.FP_for(idx) == params.stimTypes(s));

                if isFN || isFP
                    % Determine color/glyph based on type and severity.
                    % Mismatch is checked per-direction (FN:CACHE-MISMATCH vs
                    % FP:CACHE-MISMATCH) rather than testing the whole Verdict
                    % string: a combined Verdict like "FN:CACHE-MISMATCH;
                    % FP:firm-sig" would otherwise mislabel this panel's firm
                    ...
                    if isMismatchFN || isMismatchFP
                        markerColor = colorMismatch;  % black
                        markerGlyph = 'x';             % shape differs too, not just color (colorblind-safe, prints in b/w)
                        nMismatchTotal = nMismatchTotal + 1;
                    elseif isFN
                        markerColor = colorFN;         % blue
                        markerGlyph = '*';
                        nFNtotal = nFNtotal + 1;
                    else
                        markerColor = colorFP;         % orange
                        markerGlyph = '*';
                        nFPtotal = nFPtotal + 1;
                    end

                    % Plot a clean, visible marker at the end of the row
                    text(xEnd, i, markerGlyph, 'Color', markerColor, ...
    ...
    % --- Stim onset line (offset omitted when the window is a fixed truncation) ---
    xline(ax, 0, 'k', 'LineWidth', 1.5);                             % onset at t = 0 s
    if params.useCompleteWindow                                      % only draw offset when showing the full stimulus duration
        xline(ax, stimDurAll(s)/1000, 'k', 'LineWidth', 1.5);       % real stimulus end (seconds)
    end
...
% ------------------------------------------------------------------
% FP/FN legend — colors/shapes match the markers drawn in the tile loop
% above; entry labels report the total count of each marker type.
% ------------------------------------------------------------------
if params.MarkFPnFN && (nFNtotal + nFPtotal + nMismatchTotal) > 0
    hold(axAll(end), 'on');                                              % 'on' so proxy markers don't clear the raster/colorbar already on this tile
    legHandles = gobjects(1, 0);                                         % proxy handles to pass to legend()
    legLabels  = {};                                                     % matching label strings

    hFN = plot(axAll(end), nan, nan, '*', 'Color', colorFN, ...
        'MarkerSize', 8, 'LineStyle', 'none');                           % FN swatch (NaN data never actually renders)
    legHandles(end+1) = hFN;
    legLabels{end+1}  = sprintf('False Negative (n=%d)', nFNtotal);

    hFP = plot(axAll(end), nan, nan, '*', 'Color', colorFP, ...
        'MarkerSize', 8, 'LineStyle', 'none');                           % FP swatch
    legHandles(end+1) = hFP;
    legLabels{end+1}  = sprintf('False Positive (n=%d)', nFPtotal);

    if nMismatchTotal > 0
        hMismatch = plot(axAll(end), nan, nan, 'x', 'Color', colorMismatch, ...
            'MarkerSize', 8, 'LineWidth', 1.5, 'LineStyle', 'none');     % mismatch swatch (only shown if any occurred)
        legHandles(end+1) = hMismatch;
        legLabels{end+1}  = sprintf('Cache mismatch (n=%d)', nMismatchTotal);
    end
    hold(axAll(end), 'off');

    lg = legend(axAll(end), legHandles, legLabels, 'FontName', 'helvetica', ...
        'FontSize', 7, 'Box', 'off');
    lg.Layout.Tile = 'south';                                            % dedicated row below all panels — no overlap with data or colorbar
end
...
function sub = statSubStruct(Stats, stimType, speedIdx)
% statSubStruct  Return the StatisticsPerNeuron sub-struct matching a stimulus.
switch stimType
    case {"MB","MBR"}, sub = Stats.(sprintf('Speed%d', speedIdx));  % per-speed sub-field
    case "SDGm",       sub = Stats.Moving;                          % drifting phase
    case "SDGs",       sub = Stats.Static;                          % static phase
    otherwise,         sub = Stats;                                 % flat (RG, NI, NV, FFF)
end
end

function rows = preferredCatRows(sub, C, colNames, u)
% preferredCatRows  Trial rows (into C) of neuron u's stored preferred category.
%   Reproduces the exact category blocking StatisticsPerNeuron used, so the
%   displayed trials are the ones the swarm's ZScoreU was computed from.
assert(all(isfield(sub, {'prefCat','validCats','ObsDiff','catTable','focusSpec'})), ...   % standard-path decoder must be present
    'Stats sub-struct lacks the prefCat decoder (unexpected for a non-grid cache).');
c = sub.prefCat(u);                                                 % this neuron's preferred category index
if isnan(c), rows = (1:size(C,1))'; return; end                    % undefined pref -> all trials (safe fallback)

fs    = sub.focusSpec;                                              % focus spec used when the cache was built
isAll = isempty(fs.CategoryLevels) || ...                          % no separating factors named, or
    (isscalar(string(fs.CategoryLevels)) && string(fs.CategoryLevels) == "all");
focusActive = ~isempty(fs.Lock) || ~isAll;                         % lock and/or a real level subset

if focusActive                                                     % --- focus-blocked categories ---
    [keepMask, catID, catTable2] = applyCategoryFocus(C, colNames, fs.CategoryLevels, fs.Lock);  % rebuild grouping
    assert(isequal(catTable2, sub.catTable), ...                   % guard: grouping must match the cache
        'catTable mismatch — focus grouping differs from the cache.');
    keptIdx = find(keepMask);                                      % original rows surviving the lock filter
    rows    = keptIdx(catID == c);                                 % rows belonging to preferred category c
else                                                              % --- legacy contiguous blocks ---
    nCats     = size(sub.validCats, 1);                            % number of categories
    trialsCat = size(sub.ObsDiff, 1) / nCats;                     % trials per category (balanced design)
    assert(trialsCat == round(trialsCat), 'ObsDiff rows not divisible by nCats.');            % integer block size
    assert(size(C,1) == size(sub.ObsDiff,1), 'C and ObsDiff row counts differ — order mismatch.');  % same trial order
...
switch stimKey
    case {"MB","MBR"}
        if speedParam == "max" || speedParam == "best"
            fn = 'Speed1';                                           % representative field (per-neuron best speed handled in main loop)
        else
