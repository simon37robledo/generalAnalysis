    ...
    for ei = 1:nExp
        ...
        for s = 1:nStim
            ...
            if params.statType == "BootstrapPerNeuron"
                ...
                Stats = obj.StatisticsPerNeuron(SpatialGridMode=true,maxCategory=true);                         % default: permutation test
            end
        ...
        for s = 1:nStim
            ...
            for ni = 1:numel(eNeurons)
                ...
                if isCatMode
                    ...
                    % ==================================================
                    % STANDARD MODE: build PSTH from this neuron's best-speed trials
                    % ==================================================
                    prefLevel = NaN;                                     % not applicable in standard mode

                    Cu      = speedC{ bestSpeed(u) };                    % winning speed's condition matrix
                    onsetsU = Cu(:, 1)' + startStimMs;                   % best-speed onsets (ms); == directimesSorted for single-speed stim

                    % Binary spike matrix: trials x time at 1 ms resolution.
                    % windowTotal is fixed (preBase + truncated duration), so
                    % all speeds share the same length — no ragged histograms.
                    MRhist = BuildBurstMatrix( ...
                        goodU(:, u), ...                                 % spike identity for this unit
                        round(p_sort.t), ...                             % rounded sample timestamps
        ...
        for i = 1:numel(currentPhy)
            ...
            if ~isempty(idx)
                ...
                if isFN || isFP
                    ...
                    if contains(verdict, 'CACHE-MISMATCH')
                        markerColor = [1 0 0];       % Red flag for data mismatch
                    elseif isFN
                        markerColor = [0 0.7 0];     % Vibrant Green for False Negatives
                    else
                        markerColor = [1 0.5 0];     % Orange for False Positives
                    end

                    % Plot a clean, visible asterisk at the end of the row
                    text(xEnd, i, '*', 'Color', markerColor, ...
