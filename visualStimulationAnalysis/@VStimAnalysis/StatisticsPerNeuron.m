...
arguments (Input)
    ...
    params.durationWindow       = 500    % Length of moving window      
    ...
    params.CategoryLevels = "all"   % factors kept separate (their crossing = response levels);
                                    % "all" -> every parameter separate (legacy). Unnamed factors pool.
    params.Lock           = {}      % {name,value,...} lock factors to fixed values (pre-filter); {} = none
...
for s = 1:x
    ...
    if params.MovingWindowPVal && isequal(obj.stimName, 'linearlyMovingBall')
    ...
    % -------------------------------------------------------------------------
    % Category focus (optional): rebuild response categories from C.
    % Inactive by default -> legacy block layout is used unchanged.
    % -------------------------------------------------------------------------
    isAll       = isempty(params.CategoryLevels) || ...                    % no level factors named, or
        (isscalar(string(params.CategoryLevels)) && string(params.CategoryLevels)=="all");
    focusActive = ~isempty(params.Lock) || ~isAll;                        % lock and/or a real subset

    if focusActive
        % Per-condition C (the same matrix that built directimesSorted).
        if exist('fieldName','var') && isfield(responseParams, fieldName) ...
                && isfield(responseParams.(fieldName), 'C')
            Ccond = responseParams.(fieldName).C;                          % speed/phase sub-field
        else
            Ccond = responseParams.C;                                      % flat / shared
        end
        colNamesCond = responseParams.colNames{1}(5:end);                 % parameter names (strip 4 bookkeeping cols)

        % HARD alignment guard: Diff rows came from directimesSorted = C(:,1)'.
        assert(isequal(round(Ccond(:,1)'), round(directimesSorted)), ...
            'C and Diff are not row-aligned — focus grouping would misalign trials.');

        % Effective categories: lock filter + separate/pool grouping.
        [keepMask, catID, catTable, sepNames] = ...
            applyCategoryFocus(Ccond, colNamesCond, params.CategoryLevels, params.Lock);

        % Re-order kept trials into contiguous category blocks.
        keptIdx            = find(keepMask);                              % original kept row indices
        [catIDsorted, ord] = sort(catID);                                % contiguous blocks per category
        rowOrder           = keptIdx(ord);                               % final row order into original trials

        % Apply the same selection+order to every trial-indexed matrix.
        Diff      = Diff(rowOrder, :);                                    % [nKept × nNeurons]
        responses = responses(rowOrder, :);                              % keep aligned (empty-trial filter)
        baselines = baselines(rowOrder, :);                              % keep aligned
        if ~isempty(DiffPVal),                          DiffPVal = DiffPVal(rowOrder, :);    end
        if exist('DiffSeg','var') && ~isempty(DiffSeg), DiffSeg  = DiffSeg(rowOrder, :, :); end

        % New balanced block dimensions (designs are fully crossed, equal-rep).
        counts    = accumarray(catIDsorted, 1);                          % trials per category
        assert(all(counts == counts(1)), ...                             % balance required for the vectorised null
            'Focus produced unequal trials per category (%s) — design not balanced.', mat2str(counts'));
        trialsCat = counts(1);                                           % trials per effective category
    else
        catTable = [];  sepNames = strings(1,0);                         % no focus -> empty decode table
    end
   ...
   % -------------------------------------------------------------------------
    ...
    if isfield(responseParams, "Speed1") || isequal(obj.stimName, 'StaticDriftingGrating')
        ...
        S.(fieldName).catTable    = catTable;    % [nCats × nSep] level combo per category (prefCat decoder)
        S.(fieldName).catColNames = sepNames;    % [1 × nSep] separating-factor names
        S.(fieldName).focusSpec   = struct('CategoryLevels', {params.CategoryLevels}, 'Lock', {params.Lock});
    else
        ...
        S.catTable    = catTable;    % [nCats × nSep] level combo per category (prefCat decoder)
        S.catColNames = sepNames;    % [1 × nSep] separating-factor names
        S.focusSpec   = struct('CategoryLevels', {params.CategoryLevels}, 'Lock', {params.Lock});
    end
