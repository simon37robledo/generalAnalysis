...
arguments
    ...
    % --- Bin size for BuildBurstMatrix ---
    params.bin              double  = 1                     % bin size in ms. Default 1 ms gives spike
                                                            % counts at 1 ms resolution; increase (e.g.
                                                            % 10 ms) to reduce memory and compute time.
...
% =========================================================================
%  3.  EXTRACT SPIKE SORTING AND GOOD UNITS
%      (done here, before the burst matrix, since goodU is needed for it)
% =========================================================================

% Convert phy spike sorting to spike train matrix and unit metadata
p     = vsObj.dataObj.convertPhySorting2tIc(vsObj.spikeSortingFolder);
label = string(p.label');                                   % quality label per unit
goodU = p.ic(:, label == 'good');                          % [2 × nGood] spike train index matrix
phyID = p.phy_ID(label == 'good');                         % [nGood × 1] phy cluster IDs
nN    = size(goodU, 2);                                    % number of good somatic units

% =========================================================================
%  4.  EXTRACT C MATRIX AND IDENTIFY DIRECTION COLUMN
% =========================================================================

% The C matrix holds one row per trial with columns:
%   C(:,1)       = trial onset times (ms)
%   C(:,2:end)   = condition parameter values (matching colNames)
%
% colNames = rw.colNames{1}(5:end) are the condition parameter names,
% skipping the first 4 internal NeuronVals features (nSpks, duration,
% spikeRate, baseline). The offset between colNames and C is 1 because
% C(:,1) holds onset times, so colNames{k} → C(:, k+1).

% Access C from the resolved subfield
if isempty(fieldName)
    C = rw.C;                                              % flat struct (no subfield)
else
    C = rw.(fieldName).C;                                  % subfield struct (SDGm, MB, etc.)
end

colNames = rw.colNames{1}(5:end);                          % condition parameter names (cell array)

% Find the direction column: search colNames for 'direction' or 'angles'
catIdx = find(strcmpi(colNames, 'directions') | strcmpi(colNames, 'angles'), 1);
if isempty(catIdx)
    error('DirectionTuning:noDirCol', ...
        'Could not find "directions" or "angles" in colNames: {%s}.\n', ...
        strjoin(colNames, ', '));
end
catColIdx = catIdx + 1;                                    % +1 because C(:,1) is onset times

fprintf('Direction column: colNames{%d} = "%s" → C(:,%d)\n', ...
    catIdx, colNames{catIdx}, catColIdx);

% =========================================================================
%  5.  NORMALISE DIRECTION VALUES AND BUILD TUNING CURVE
% =========================================================================

dirRaw = C(:, catColIdx);                                  % raw direction values per trial [nTrials × 1]

% Normalise direction values to radians in [0, 2π).
%
% SDGm / SDGs require a remapping step before normalisation because gratings
% store directions in a different degree convention than MB:
%   SDG 0°   → π/2   rad  (MB "up")
%   SDG 90°  → 0     rad  (MB "right")
%   SDG 180° → 3π/2  rad  (MB "down")
%   SDG 270° → π     rad  (MB "left")
% Without this, OSI/DSI would be numerically correct for each stimulus
% independently but the preferred-direction angles would not be comparable
% across SDG and MB in cross-stimulus analyses.
if contains(class(vsObj), 'StaticDrifting', 'IgnoreCase', true)
    ...
    dirRemapped = nan(size(dirRaw));                        % pre-fill with NaN to catch unmapped values
    for k = 1:size(sdgMap, 1)
        mask = abs(dirRaw - sdgMap(k, 1)) < 1e-6;          % trials at this SDG degree value
        dirRemapped(mask) = sdgMap(k, 2);                  % assign the corresponding MB radian
    end

    % Warn if any trial has a direction value not covered by the map
    if any(isnan(dirRemapped))
        warning('DirectionTuning:unmappedSDGdir', ...
            'Some SDGm/SDGs direction values could not be remapped: [%s] deg. ' + ...
            'Extend sdgMap if the stimulus uses additional directions.', ...
            num2str(unique(dirRaw(isnan(dirRemapped)))', '%.4g '));
    end

    dirNorm = mod(dirRemapped, 2*pi);                       % normalise to [0, 2π); values already radians
    fprintf('SDGm/SDGs: directions remapped to MB radian convention.\n');

else
    % All other stimuli: detect whether values are in degrees or radians.
    % If any absolute value exceeds 2π + a small tolerance, assume degrees.
    if max(abs(dirRaw)) > 2*pi + 0.1
        dirNorm = mod(deg2rad(dirRaw), 2*pi);               % convert degrees → radians, normalise
        fprintf('Directions detected as degrees → converted to radians.\n');
    else
        dirNorm = mod(dirRaw, 2*pi);                        % already radians, normalise to [0, 2π)
    end
end

% Unique directions after normalisation.
% uniquetol groups values within tol of each other — handles the case where
% 0 and 2π (or 0 and 360°) are stored as distinct but equivalent values.
dirTol  = 1e-4;                                            % tolerance in radians (~0.006°)
uDirRad = uniquetol(dirNorm, dirTol)';                     % [1 × nDir] sorted unique directions
nDir    = numel(uDirRad);                                  % number of unique directions

assert(nDir >= 3, 'DirectionTuning:tooFewDirs', ...
...
% Get stimulus duration and inter-stimulus interval from ResponseWindow
stimDur   = rw.(fieldName).stimDur;                        % stimulus duration (ms)
stimInter = rw.stimInter;                                  % inter-stimulus interval (ms)

% Build the burst matrix from raw spike trains.
% Arguments (all in bins = ms when params.bin == 1):
%   goodU               : spike train index matrix
%   round(p.t/bin)      : total recording length in bins
%   round(C(:,1)/bin)   : trial onset times in bins
%   round(stimDur/bin)  : window size in bins (response window = stimulus duration)
%
% NOTE: No pre-stimulus offset is added here — window starts at trial onset.
%   If a baseline period is needed, extend the window and offset trial times.
%   For OSI/DSI we only need the response during the stimulus.
%
...
tuningCurve    = zeros(nN, nDir);                          % [nN × nDir] mean spike rate (spks/s)
tuningCurveSEM = zeros(nN, nDir);                          % [nN × nDir] SEM across trials
nTrialsPerDir  = zeros(1,  nDir);                          % [1  × nDir] trial count per direction

for d = 1:nDir
    % Logical mask: trials belonging to this direction (within tolerance)
    trialMask = abs(dirNorm - uDirRad(d)) < dirTol;        % [nTrials × 1] logical
    nTr       = sum(trialMask);                            % number of trials for this direction

    % Sum spike counts across all bins within the stimulus window.
    % Mr(trialMask, :, :) is [nTr × nN × nBins]; sum over dim 3 → [nTr × nN].
    spikeCounts = sum(Mr(trialMask, :, :), 3);            % [nTr × nN] total spikes per trial

    % Convert spike counts to firing rate (spikes/s)
    spikeRates_d = spikeCounts / (stimDur / 1000);         % [nTr × nN] divide by duration in seconds

    % Mean spike rate across trials for each neuron
    tuningCurve(:, d) = mean(spikeRates_d, 1)';           % [nN × 1]

    % SEM across trials (undefined for n=1, set to 0)
    if nTr > 1
        tuningCurveSEM(:, d) = std(spikeRates_d, 0, 1)' / sqrt(nTr);  % [nN × 1]
    end

    nTrialsPerDir(d) = nTr;                                % store trial count for diagnostics
end
...
% =========================================================================
%  11. GOOD-UNIT IDENTIFIERS
%      (goodU and phyID were extracted in section 3 alongside p_sort)
% =========================================================================
% goodU and phyID are already defined above.
...
% (findDirectionColumn removed: direction column detection is now inline
%  in section 4 using the C matrix and colNames directly.)
